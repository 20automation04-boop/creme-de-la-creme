...
- **Automated replay tests** — `npm test` runs 34 fixture-driven conversations (`test/replays/*.json`) through the real message handler under `BOT_DRY_RUN=1`, covering the ordering FSM, button/list routing, stale-tap handling, owner commands, idle expiry + resume, and the escalation ladder. Plus `test/menu-sheet.test.js` for the sheet-driven menu CRUD, and `test/security.test.js` for the HTTP surface (webhook signature enforcement, verify-token handling, kitchen login lockout, cookie flags).
- **Tap-to-add ordering** — selecting an item adds one of it immediately and leaves the category list up, so picking three things is three taps instead of nine round trips. Quantities are collected in a single pass at checkout (the `qtybatch` step), one question per item, answerable by button or by typing — and a typed answer can carry a customisation in the same reply ("2 no onions"), which is why the per-item note prompt could be dropped without losing anything. A quantity the customer stated outright (the `2x3` shorthand, a typed order, a repeat, the add-on button) is never re-asked.
